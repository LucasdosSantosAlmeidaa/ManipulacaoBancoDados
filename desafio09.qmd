...
Este laboratório demonstra o uso de SQLite para processamento de dados em lote, utilizando chunks para leitura eficiente de arquivos grandes sem sobrecarregar a memória RAM.
...
Criamos o arquivo de banco de dados `voos.sqlite3`. Se o arquivo não existir, será criado automaticamente. Se já existir, a conexão será estabelecida.
...
Carregamos os arquivos `airlines.csv` e `airports.csv` nas respectivas tabelas do banco de dados. Estas tabelas contêm informações de referência que serão usadas posteriormente em JOINs.
...
Criamos a função que processa cada chunk de dados, filtrando apenas os voos que partem ou chegam aos aeroportos de interesse e salvando-os no banco.
...
Lemos o arquivo grande em chunks (lotes) de 100 mil registros por vez, aplicando a função lerDados a cada chunk. Esta abordagem permite processar arquivos maiores que a memória RAM disponível.
...
Consultamos o tempo médio de atraso por aeroporto de destino, combinando informações das três tabelas (flights, airports, airlines) através de JOINs.
...
Apresentamos estatísticas gerais sobre os dados processados para validação e entendimento do conjunto de dados.
...
Desconectamos do banco de dados para liberar recursos e finalizar a sessão de forma adequada.
